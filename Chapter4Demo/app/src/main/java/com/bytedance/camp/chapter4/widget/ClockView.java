...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        // 绘制时针
        canvas.save();
        canvas.rotate(360 / 12 * hour, centerX, centerY);
        needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
        canvas.drawLine(centerX, centerY, centerX, centerY - HOUR_NEEDLE_LENGTH_RATIO * radius, needlePaint);
        canvas.restore();
        // 绘制分针
        canvas.save();
        canvas.rotate(360 / 60 * minute, centerX, centerY);
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        canvas.drawLine(centerX, centerY, centerX, centerY - MINUTE_NEEDLE_LENGTH_RATIO * radius, needlePaint);
        canvas.restore();
        //绘制秒针
        canvas.save();
        canvas.rotate(360 / 60 * second, centerX, centerY);
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        canvas.drawLine(centerX, centerY, centerX, centerY - SECOND_NEEDLE_LENGTH_RATIO * radius, needlePaint);
        canvas.restore();
    ...
    private void drawTimeNumbers(Canvas canvas) {
        ...
        Rect rect = new Rect();
        for(int i = 0; i < 12; i++) {
            numberPaint.getTextBounds(str[i], 0, str[i].length(),rect);
            canvas.drawText(str[i], centerX - rect.width()/2, centerY - radius + 40 + rect.height(),numberPaint);
